...
import { searchBookList } from "../../store/slices/recipeSlice";
      ...
      {account ? (
        <div className="row pt-2 pb-2">
          ...
          <div className="col-12">
            <h1>Welcome {user?.currentUser?.firstname || ""} {user?.currentUser?.lastname || ""}</h1>
        ...
        {recipes?.length > 0 ?
          ...
          recipes?.map((recipe, i) => (
            <div className="col-lg-4 col-md-6" key={i}>
              <BooksCard recipe={recipe} user={user} account={account} />
            </div>
          ))
